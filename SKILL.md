...
- `markdown`：按类别输出表格（见「输出格式」），每条须含「官网 URL」列，并附「证据来源」区块。
- `json`：返回对象数组，每条含字段 `category, name, benefit, entry, validity, limits, evidence`，结构与表格列一一对应；其中 `entry` 为**官网 URL（必填、完整 https 链接）**。
...
**API 类**
| 模型/平台名称 | 免费权益详情 | 官网 URL | 接入方式 | 免费有效期 | 使用限制 | 证据来源 |
|---|---|---|---|---|---|---|
...
- **官网 URL**：平台官网或模型主页的完整 https 链接（**必填**，每条记录都必须提供；缺失视为不合格，补全后再输出）
- **接入方式**：API 接入方式（如 OpenAI 兼容 base_url、SDK 名称）或「网页直接使用」
...
- 不编造不存在的免费资源。
- 🔴 **每条记录必须包含「官网 URL」列，且为非空的完整 https 链接**；若检索结果未含网址，须用 `{{web_fetch}}` 补全后再输出（缺失官网 URL 的记录不得输出）。
...
[
  {
    ...
    "name": "模型/平台名称",
    "benefit": "免费权益详情",
    "entry": "官网 URL（必填，完整 https 链接）",
    "access": "接入方式（OpenAI 兼容 base_url / SDK 名称 / 网页直接使用）",
